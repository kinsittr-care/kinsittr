  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        ...
        align="end"
        sideOffset={10}
        className="w-[calc(100vw-2rem)] max-w-80 overflow-hidden p-0"
      >
        {/* Header */}
          ...
          {!listQuery.isError && notifications.map((notif) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full min-w-0 flex items-start gap-3 overflow-hidden px-4 py-3 text-left border-b border-border/50 transition-colors",
              ...
              >
                ...
                <div className="min-w-0 flex-1 overflow-hidden">
                  <p className={cn("line-clamp-2 break-words text-[13px] text-foreground mb-0.5", isUnread ? "font-semibold" : "font-medium")}>
                    {notif.title}
                  </p>
                  <p className="line-clamp-2 break-words text-xs text-brand-faint">{notif.body}</p>
                </div>
