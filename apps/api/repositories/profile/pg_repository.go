...
func (r *pgRepository) CreateNannyProfile(ctx context.Context, p models.NannyProfile) (models.NannyProfile, error) {
	...
	err := r.db.QueryRow(ctx, `
		...
		RETURNING id, user_id, display_name, bio, rate_per_hour, service_type, currency, verification_status, city, province, created_at, updated_at
	`,
		p.ID, p.UserID, p.DisplayName, p.Bio, p.RatePerHour, p.ServiceType, p.Currency, p.City, p.Province,
	).Scan(
...
func (r *pgRepository) GetNannyProfileByUserID(ctx context.Context, userID uuid.UUID) (models.NannyProfile, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT id, user_id, display_name, bio, rate_per_hour, service_type, currency, verification_status, city, province, created_at, updated_at
