...
func scanAdminInvite(row pgx.Row) (models.AdminInvite, error) {
...
func (r *pgRepository) CreateAdminInvite(ctx context.Context, params InviteAdminParams) (models.AdminInvite, error) {
	if params.ID == uuid.Nil {
		params.ID = uuid.New()
	}
	if params.ExpiresAt.IsZero() {
		params.ExpiresAt = time.Now().UTC().Add(72 * time.Hour)
	}
	return scanAdminInvite(r.db.QueryRow(ctx, `
		INSERT INTO admin_invites (id, firstname, lastname, email, token_hash, invited_by, expires_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id, firstname, lastname, email, token_hash, invited_by, accepted_by, accepted_at, expires_at, created_at
	`, params.ID, params.Firstname, params.Lastname, params.Email, params.TokenHash, params.InvitedBy, params.ExpiresAt))
}

func (r *pgRepository) AcceptAdminInvite(ctx context.Context, params AcceptAdminInviteParams) (AdminUserRecord, error) {
	...
	invite, err := scanAdminInvite(tx.QueryRow(ctx, `
		SELECT id, firstname, lastname, email, token_hash, invited_by, accepted_by, accepted_at, expires_at, created_at
		FROM admin_invites
		WHERE token_hash = $1
	...
	`, adminID, invite.Firstname, invite.Lastname, invite.Email, params.PasswordHash, models.AdminUserRole))
	if err != nil {
		return AdminUserRecord{}, err
	}
