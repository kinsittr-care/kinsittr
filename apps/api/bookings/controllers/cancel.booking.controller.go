...
func (c *BookingsController) Cancel(ctx *fiber.Ctx) error {
	...
	res := c.pipe.Cancel(ctx.Context(), userID, bookingID)
	if !res.Success {
		status := fiber.StatusBadRequest
		switch string(res.Message) {
		case messages.Parent_Profile_Not_Found, messages.Booking_Not_Found:
			status = fiber.StatusNotFound
		case messages.Booking_Already_Approved:
